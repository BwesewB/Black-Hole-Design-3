...
const VideoBackground = forwardRef(({ src, loop = false, onVideoEnd }, ref) => {
  const videoRef1 = useRef(null); // Reference to the main video
  const videoRef2 = useRef(null); // Reference to the transition video
  const [isMainActive, setIsMainActive] = useState(true); // Track which video is active
  const [isTransitioning, setIsTransitioning] = useState(false); // Track transition state
  const [videoLoaded, setVideoLoaded] = useState(false); // Track if video has loaded correctly
  ...
  useImperativeHandle(ref, () => ({
    ...
    playNextVideo(newSrc, newLoop, onEndCallback) {
      ...
      const handleLoadedMetadata = () => {
        console.log(`Video metadata loaded: ${newSrc}`);
        setVideoLoaded(true); // Set video as loaded when metadata is loaded
      ...
      transitionVideo.onloadedmetadata = handleLoadedMetadata; // Use loadedmetadata to ensure the video is ready
  ...
  return (
    <>
      <video
        ...
        onLoadedMetadata={() => setVideoLoaded(true)} // Track when metadata is loaded
        style={{
          position: "fixed",
          top: 0,
          left: 0,
