  ...
  const handleNextPage = (nextPage, videoConfig) => {
    ...
    gsap.to(".current-page", {
      onComplete: async () => {
        ...
        try {
          // Play the transition video, if applicable
          if (transitionVideo) {
            await videoRef.current.playNextVideo(transitionVideo, false);
  
            // Once the transition video is done, play the main video
            await videoRef.current.playNextVideo("/videos/Clip16.mp4", true);
          } else {
            // Directly play the main video
            await videoRef.current.playNextVideo(videoConfig.src, videoConfig.loop, videoConfig.onVideoEnd);
          }
        } catch (error) {
