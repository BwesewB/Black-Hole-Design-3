...
const VideoBackground = forwardRef(({ src, loop = false, onVideoEnd }, ref) => {
  ...
  useImperativeHandle(ref, () => ({
    ...
    playNextVideo(newSrc, newLoop, onEndCallback) {
      ...
      // Listen for the video 'pause' event to handle stuck scenarios
      transitionVideo.onpause = () => {
        console.warn(`Video paused unexpectedly: ${newSrc}`);
        handleError();
      };

      // Listen for the video 'play' event to ensure it's playing
      transitionVideo.onplay = () => {
        console.log(`Video started playing: ${newSrc}`);
      };

      // Listen for 'stalled' event to detect video load issues
      transitionVideo.onstalled = () => {
        console.warn(`Video stalled: ${newSrc}, retrying...`);
        handleError();
      };
    }
  }));
